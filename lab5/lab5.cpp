...
HRESULT InitScene()
{
    ...
    if (SUCCEEDED(result))
    {
        const std::wstring TextureName = L"puppy.dds";
...
HRESULT InitSmallSphere()
{
    ...
    if (SUCCEEDED(result))
    {
        result = CompileAndCreateShader(L"SphereTexture.ps", (ID3D11DeviceChild**)&m_pSmallSpherePixelShader);
    }
...
HRESULT InitRect()
{
    ...
    if (SUCCEEDED(result))
    {
        ...
        geomBuffer.color = Point4f{ 0.75f, 0, 0.05f, 1.0f };
        ...
        if (SUCCEEDED(result))
        {
            ...
            geomBuffer.color = Point4f{ 0.5f, 0.6f, 0.15, 1.0f };
...
void RenderSmallSphere()
{
    ...
    ID3D11ShaderResourceView* resources[] = { m_pSmallCubemapView };
    g_pd3dDeviceContext->PSSetShaderResources(0, 1, resources);
